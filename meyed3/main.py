...
import meyed3
import meyed3.__about__
import meyed3.plugins
import meyed3.utils
import meyed3.utils.console
from meyed3.utils.log import initLogging
...
def main(args, config):
    ...
    for p in args.paths:
        meyed3.utils.walk(args.plugin, p, excludes=args.excludes, fs_encoding=args.fs_encoding,
                          recursive=recursive)
...
def _listPlugins(config):
    from meyed3.utils.console import Fore, Style
    ...
    all_plugins = meyed3.plugins.load(reload=True, paths=_getPluginPath(config))
...
@deprecation.deprecated(deprecated_in="0.9a2", removed_in="1.0",
                        current_version=meyed3.__about__.__version__,
    ...
    for config_file in config_files:
        if os.path.isfile(config_file):
            ...
            try:
                ...
                meyed3.log.warning(f"User config error: {ex}")
                return None
            else:
                return config
...
def profileMain(args, config):  # pragma: no cover
    ...
    meyed3.log.debug("driver profileMain")
...
def setFileScannerOpts(arg_parser, default_recursive=False, paths_metavar="PATH",
    ...
    arg_parser.add_argument("--fs-encoding", action="store", dest="fs_encoding",
                            default=meyed3.LOCAL_FS_ENCODING, metavar="ENCODING",
                            help="Use the specified file system encoding for filenames. "
                                 f"Default as it was detected is '{meyed3.LOCAL_FS_ENCODING}' but "
...
def makeCmdLineParser(subparser=None):
    from meyed3.utils import ArgumentParser

    p = ArgumentParser(prog=meyed3.__about__.__project_name__, add_help=True) \
...
def parseCommandLine(cmd_line_args=None):
    ...
    PluginClass = meyed3.plugins.load(plugin_name, paths=_getPluginPath(config))
    if PluginClass is None:
        meyed3.utils.console.printError("Plugin not found: %s" % plugin_name)
        parser.exit(1)
    ...
    args.plugin = plugin
    meyed3.log.debug("command line args: %s", args)
    meyed3.log.debug("plugin is: %s", plugin)
...
def _main():
    ...
    try:
        ...
        meyed3.utils.console.AnsiCodes.init(not args.no_color)
        ...
        meyed3.utils.console.printError(str(ex))
        retval = 1
    except Exception as ex:
        meyed3.utils.console.printError(f"Uncaught exception: {ex}\n")
        meyed3.log.exception(ex)
        retval = 1
