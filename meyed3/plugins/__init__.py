...
from meyed3 import core, utils
from meyed3.utils import guessMimetype, formatSize
from meyed3.utils.console import printMsg, printError, HEADER_COLOR, boldText, Fore
...
def load(name=None, reload=False, paths=None):
    """Returns the meyed3.plugins.Plugin *class* identified by ``name``.
            ...
            for f in os.listdir(d):
                ...
                for attr in [getattr(mod, a) for a in dir(mod)]:
                    if type(attr) == type and issubclass(attr, Plugin):
                        # This is a meyed3.plugins.Plugin
                        PluginClass = attr
...
class LoaderPlugin(Plugin):
    ...
    def handleFile(self, f, *args, **kwargs):
        ...
        :class:`meyed3.core.AudioFile` or ``None`` if an error occurred or the
        file is not a recognized type.

        The ``*args`` and ``**kwargs`` are passed to :func:`meyed3.core.load`.
        """
